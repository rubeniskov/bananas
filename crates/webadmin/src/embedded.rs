//! Serve the webadmin SPA bundle from bytes embedded into this binary.
//!
//! Replaces the old `tower_http::ServeDir::new(/usr/share/bananas/
//! webadmin).precompressed_br().precompressed_gzip()` setup. The
...
//!  - **/assets/<hash>.<ext>** lookups: exact-path match in the
...
//!  - **SPA fallback**: any path that doesn't match (e.g. /, a
//!    deep-link like /#cloud) returns `index.html`, also accept-
//!    encoding-negotiated.
...
///
/// Caller passes the request path relative to the SPA's own root
/// (e.g. `assets/main-<hash>.css` or `index.html`); webadmin mounts
/// at `/`, so the daemon hands the URI through unchanged.
