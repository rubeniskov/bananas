//! BanaNAS Dashboard config SPA — composed into the webadmin
//! shell at runtime. Renders the appearance/refresh-rate form
//! for /etc/bananas/dashboard.toml.
    ...
    rsx! {
        ...
        match state() {
            ...
            AuthState::SignedIn => rsx! {
                if mfe {
                    div { class: "section-header", h2 { "Dashboard config" } }
                    dashboard_config::DashboardConfigForm {}
                } else {
                    main {
                        nav { class: "app-nav",
                            h1 { class: "app-title", "BanaNAS Dashboard" }
                            span { class: "spacer" }
                            a {
