//! Serve the stats SPA's static assets from bytes embedded into
//! this binary. **Strict asset lookup only** — `index.html` is
//! never publicly served. Webadmin owns THE single document root;
//! this daemon answers only `GET /assets/stats/<rest>` requests
...
/// `/api/stats/__mfe_entry` JSON endpoint so the host SPA's MFE
/// loader knows which content-hashed JS shim to inject.
///
/// Dioxus.toml's `base_path = "/assets/stats"` causes dx to emit
/// `<script type="module" src="/assets/stats/bananas-stats-ui-<hash>.js">`,
...
pub static MFE_ENTRY: LazyLock<String> = LazyLock::new(|| {
    ...
    extract_module_src(html)
        .expect("no <script type=\"module\" src=\"…\"> in embedded stats-ui index.html")
});

/// Serve `<rest>` from the embedded UI tree. The caller hands us
/// the request path with the `/assets/stats/` prefix already
/// stripped, so `rest` looks like:
///  - `assets/bananas-stats-ui-<hash>.js` (dx emits hashed files
///    under its own `assets/` subdir; combined with our
///    `base_path = "/assets/stats"`, the public URL becomes
///    `/assets/stats/assets/<hash>.js`)
///  - `wasm/bananas-stats-ui_bg.wasm` (older dx layout)
...
/// `/assets/stats/`); we return it as-is.
...
mod tests {
    ...
    fn extracts_module_src_double_quoted() {
        let html = r#"<!doctype html><html><body>
            <div id="main"></div>
            <script type="module" src="/assets/stats/bananas-stats-ui-deadbeef.js"></script>
        </body></html>"#;
        assert_eq!(
            extract_module_src(html).as_deref(),
            Some("/assets/stats/bananas-stats-ui-deadbeef.js")
        );
    }

    #[test]
    fn extracts_module_src_single_quoted() {
        let html = "<script type='module' src='/assets/stats/foo.js'></script>";
        assert_eq!(
            extract_module_src(html).as_deref(),
            Some("/assets/stats/foo.js")
        );
    }
    ...
    fn ignores_non_module_scripts() {
        let html = r#"
            ...
            <script type="module" src="/assets/stats/right.js"></script>
        "#;
        assert_eq!(
            extract_module_src(html).as_deref(),
            Some("/assets/stats/right.js")
        );
    }
