//! DiskCard — the single-disk render block reused by stats.rs.
//! The original `StoragePage` (fstab editor) lives in the
//! bananas-storage plugin now; only the disk-rendering primitive
//! stays here for the stats dashboard.
