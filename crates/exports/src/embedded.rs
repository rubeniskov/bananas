//! Serve the exports SPA's static assets from bytes embedded into
//! this binary. **Strict asset lookup only** — `index.html` is
//! never publicly served. Webadmin owns THE single document root;
//! this daemon answers only `GET /assets/exports/<rest>` requests
...
/// `/api/exports/__mfe_entry` JSON endpoint so the host SPA's MFE
/// loader knows which content-hashed JS shim to inject.
///
/// Dioxus.toml's `base_path = "/assets/exports"` causes dx to emit
/// `<script type="module" src="/assets/exports/bananas-exports-ui-<hash>.js">`,
...
pub static MFE_ENTRY: LazyLock<String> = LazyLock::new(|| {
    ...
    extract_module_src(html)
        .expect("no <script type=\"module\" src=\"…\"> in embedded exports-ui index.html")
});

/// Serve `<rest>` from the embedded UI tree. The caller hands us
/// the request path with the `/assets/exports/` prefix already
/// stripped, so `rest` looks like:
///  - `assets/bananas-exports-ui-<hash>.js` (dx emits hashed files
///    under its own `assets/` subdir; combined with our
///    `base_path = "/assets/exports"`, the public URL becomes
///    `/assets/exports/assets/<hash>.js`)
///  - `wasm/bananas-exports-ui_bg.wasm` (older dx layout)
...
/// `/assets/exports/`); we return it as-is.
...
mod tests {
    ...
    fn extracts_module_src_double_quoted() {
        let html = r#"<!doctype html><html><body>
            <div id="main"></div>
            <script type="module" src="/assets/exports/bananas-exports-ui-deadbeef.js"></script>
        </body></html>"#;
        assert_eq!(
            extract_module_src(html).as_deref(),
            Some("/assets/exports/bananas-exports-ui-deadbeef.js")
        );
    }

    #[test]
    fn extracts_module_src_single_quoted() {
        let html = "<script type='module' src='/assets/exports/foo.js'></script>";
        assert_eq!(
            extract_module_src(html).as_deref(),
            Some("/assets/exports/foo.js")
        );
    }
    ...
    fn ignores_non_module_scripts() {
        let html = r#"
            ...
            <script type="module" src="/assets/exports/right.js"></script>
        "#;
        assert_eq!(
            extract_module_src(html).as_deref(),
            Some("/assets/exports/right.js")
        );
    }
