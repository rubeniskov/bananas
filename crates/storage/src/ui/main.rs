//! BanaNAS Storage SPA — composed into the webadmin shell at
//! runtime via the MFE flow. Reuses the same session cookie.
//!
//! Today the page is the /etc/fstab editor (mount-points
//! management). Disk usage and SMART status moved to the Stats
//! plugin. The PermissionsModal is bundled here because the
//! mount-points form lets operators chown/chmod a freshly created
//! mountpoint inline — see `mounts::MountsSection` and
