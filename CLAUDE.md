...
pixi run build      # full image build via kas (kas build kas-bpi.yml) — bananas-bpi default
pixi run build-rpi  # bake the unified RPi image instead (kas build kas-rpi.yml)
...
- **`crates/webadmin`** — unprivileged HTTP daemon (`bananas` user, port 8080). Pure JSON API at `/api/*` plus a custom axum handler (see `src/embedded.rs`) that serves the SPA from bytes embedded at compile time via `include_dir!("$OUT_DIR/ui")`. The handler does Accept-Encoding negotiation (`.br` then `.gz` then raw) and falls back to `index.html` for SPA routes. A second `[[bin]]` named `bananas-webadmin-ui` is the Dioxus 0.7 wasm SPA (sources under `src/ui/`); it's gated by `required-features = ["wasm-ui"]` so workspace-native cargo check skips it. The daemon's `build.rs` runs `dx build --bin bananas-webadmin-ui --features wasm-ui --platform web`, copies the dist tree into `$OUT_DIR/ui/`, pre-compresses each asset with brotli + gzip, and `include_dir!` picks the result up.
- **`crates/cloud`** — same shape as webadmin: one Cargo package with the daemon at `src/main.rs` and the SPA at `src/ui/main.rs` (bin `bananas-cloud-ui`), embedded the same way. Mounted under `/cloud/*` by bananas-router. The host webadmin's MFE loader (`crates/webadmin/src/ui/mfe.rs`) fetches `/cloud/index.html` to discover the cloud SPA's content-hashed JS shim and injects a `<script type="module">` tag at runtime, so the cloud admin renders inline inside the webadmin shell.

Cross-compile pipeline (`pixi run build-webadmin-arm`) uses `cargo-zigbuild` for the armv7 binaries and runs the `bananas-webadmin` + `bananas-engine` + `bananas-router` + `bananas-cloud` packages (`-p`-pinned). Each daemon's `build.rs` invokes dx as a side effect, so `iterate` no longer chains a separate `build-webadmin`/`build-cloud-ui` wasm task — the daemon cross-compile produces both. The build.rs strips `cargo-zigbuild`'s armv7-targeted env vars (CARGO_TARGET_ARMV7_*, RUSTFLAGS, …) before invoking dx so the recursive cargo-for-wasm32 build inside dx isn't poisoned by the outer host's cross-compile flags.
...
```bash
pixi shell
# Pick the overlay matching the MACHINE you want — kas-bpi.yml for bananas-bpi,
# kas-rpi.yml for bananas-rpi.
kas shell kas-bpi.yml -c 'bitbake <recipe>'              # e.g. linux-sunxi, u-boot-sunxi, bananas-image
kas shell kas-bpi.yml -c 'bitbake -c <task> <recipe>'    # single task: cleansstate, compile, devshell, ...
```

`kas` materialises the upstream layers (`poky/`, `meta-openembedded/`, `meta-sunxi/`, `meta-arm/`, `meta-raspberrypi/`) into the repo root and writes everything else under `build/` (`build/conf/`, `build/tmp/`, `build/downloads/`, `build/sstate-cache/`). Both the cloned layers and `build/` are gitignored — never commit them.

## Architecture

### Build composition (kas-base.yml + per-machine overlays)

The kas tree is split into a board-agnostic base and per-machine overlays:

- `kas-base.yml` — distro, target, env block, upstream layer pins (`poky`, `meta-openembedded`, `meta-arm`, `meta-bananas`), and the shared `local_conf_header` blocks. **No** machine, **no** board-specific layers.
- `kas-bpi.yml` — `header.includes: [kas-base.yml]`, sets `machine: bananas-bpi`, pulls in `meta-sunxi`, sets `IMAGE_FSTYPES = "wic tar.gz"` and the sun7i serial console.
- `kas-rpi.yml` — `header.includes: [kas-base.yml]`, sets `machine: bananas-rpi`, pulls in `meta-raspberrypi`, sets `IMAGE_FSTYPES = "rpi-sdimg tar.gz"`, `RPI_USE_U_BOOT = "1"`, and the RPi serial / firmware-license bits.

Every kas invocation MUST point at one of the two overlays (`kas build kas-bpi.yml` or `kas build kas-rpi.yml`). There is no top-level `kas.yml` shim — picking a board is explicit.
...
If a build setting needs to be global (applies to every MACHINE), edit `kas-base.yml`'s `local_conf_header` rather than touching `build/conf/local.conf` directly — the latter is regenerated by kas. Per-machine knobs (e.g. `IMAGE_FSTYPES`, `SERIAL_CONSOLES`) belong in the matching overlay (`kas-bpi.yml` or `kas-rpi.yml`).
...
- Yocto release line is `scarthgap`. If you bump it, update every `refspec` in `kas-base.yml` (and the per-board overlays for board-specific layers like `meta-sunxi` / `meta-raspberrypi`) together, then re-test the bbappends — version-pinned appends like `elfutils_0.191.bbappend` and `dtc_1.7.0.bbappend` will silently stop applying when the upstream recipe version changes.
- **Watch for wrynose (Yocto 6.0 LTS) branches to ship.** Verified 2026-05-02: poky/meta-openembedded/meta-sunxi/meta-arm/meta-raspberrypi only have branches up to walnascar; wrynose docs exist but the branches don't yet. When `git ls-remote https://git.yoctoproject.org/poky refs/heads/wrynose` returns a SHA, revisit the bake stack: bump every refspec in `kas-base.yml` + `kas-bpi.yml` + `kas-rpi.yml` to `wrynose`, drop the `meta-lts-mixins` repo entry (rust 1.94+ ships natively), and audit the version-pinned bbappends. Until then we stay on scarthgap with `meta-lts-mixins/scarthgap/rust` pinning rust 1.92.
