...
- **`crates/server`** — unprivileged HTTP daemon (`bananas` user, port 8080). Pure JSON API at `/api/*` plus `tower-http::ServeDir` serving the wasm SPA from `BANANAS_UI_DIR` (default `/usr/share/bananas/ui`). The fallback is `index.html` so the SPA owns its own routing.
- **`crates/ui`** — Dioxus 0.7 Web app (wasm32). Compiled with `dx bundle --release --platform web` via the `pixi run build-ui` task. Output lands under `target/dx/bananas-ui/release/web/public/` and is staged into `serve/ui/` for the Yocto recipe to install at `/usr/share/bananas/ui/`.

Cross-compile pipeline (`pixi run build-server-arm`) uses `cargo-zigbuild` for the armv7 binaries and runs **only** the `bananas-server` + `bananas-helper` packages (`-p`-pinned, since `bananas-ui` is wasm-only). The `pixi run build-ui` task is a separate step that the `iterate` loop chains in front. The `bananas-server.bb` recipe `bbfatal`s if `serve/ui/index.html` is missing, so forgetting `build-ui` fails loudly instead of shipping a 404-only image.
